  ...
  (define testbench-source
    (format (file->string (build-path (getenv "LAKEROAD_DIR") "racket" testbench-filename))
            ...
            (if (>= (length (symbolics bv-expr)) 2)
                (format "top->~a = ((~a)&~a);" (second (symbolics bv-expr)) (- (expt 2 (bvlen (first (symbolics bv-expr)))) 1) (second (symbolics bv-expr)))
                "")
            (if (>= (length (symbolics bv-expr)) 3)
                (format "top->~a = ((~a)&~a);" (third (symbolics bv-expr)) (- (expt 2 (bvlen (first (symbolics bv-expr)))) 1) (third (symbolics bv-expr)))
                "")
            (if (>= (length (symbolics bv-expr)) 4)
                (format "top->~a = ((~a)&~a);" (fourth (symbolics bv-expr)) (- (expt 2 (bvlen (first (symbolics bv-expr)))) 1) (fourth (symbolics bv-expr)))
                "")
            (if (>= (length (symbolics bv-expr)) 5)
                (format "top->~a = ((~a)&~a);" (fifth (symbolics bv-expr)) (- (expt 2 (bvlen (first (symbolics bv-expr)))) 1) (fifth (symbolics bv-expr)))
                "")
            (if (>= (length (symbolics bv-expr)) 6)
                (format "top->~a = ((~a)&~a);" (sixth (symbolics bv-expr)) (- (expt 2 (bvlen (first (symbolics bv-expr)))) 1) (sixth (symbolics bv-expr)))
                "")
            (if (>= (length (symbolics bv-expr)) 1)
