...
;;; expressions, but also *simulated* at the level of Verilator simulation. This helper macro does
;;; the verification component, with its main contribution being that it keeps the Rosette symbolic
;;; state clean by wrapping everything in `with-vc` and `with-terms`. It also optionally builds a
;;; list of simulation tasks. Note: this macro does not actually run simulation. Simulation should
;;; be run in a batch after verification using `simulate-with-verilator`. Simulating in a batch is
;;; much faster than simulating each test individually, as we can parallelize Verilation/C++
;;; compilation.
;;;
;;; Args:
;;; - name: Name of the test. A string.
;;; - symbolic-define-etc: (define-symbolic ...) statements, plus other statements needed to prepare
;;;     the verification environment (e.g. any assert or assume statements you might need).
...
(define-syntax verify-lakeroad-expression
  (syntax-rules ()
    ...
    [(_ name symbolic-define-etc ... lr-expr bv-expr add-f)
     (test-true
    ...
    [(_ name symbolic-define-etc ... lr-expr bv-expr)
     (verify-and-add name symbolic-define-etc ... lr-expr bv-expr identity)]))
