#lang racket/base
;;; This is the DSP implementation generated via:
...
;;;
;;; BUT with an important modification: the use of hash-union is removed, and the state hash is
;;; assumed to come from the A signal.
;;;
;;; This is done because hash-union isn't symbolically lifted in Rosette. This is a hacky workaround.
...
(define xilinx-ultrascale-plus-dsp48e2
  (λ (#:A (A (bv->signal (constant (list 'A 'symbolic-constant) (bitvector 30))))
    ...
    (let* (;[merged-input-state-hash (hash)]
           ...
           [btor1 (bitvector 30)]
           [btor2 A]
           [merged-input-state-hash (signal-state A)]
