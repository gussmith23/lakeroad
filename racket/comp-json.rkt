...
;       function
(define (hasheq-helper #:base [base 'nil] #:valfn [valfn (lambda (v) v)] . vals)
...
;          `make-cells`
; + NETNAMES: TODO: I don't know why this exists outside of PORTS.
;
...
(define (add-module-to-doc doc mod-name mod)
  (let* ([modules (get-modules doc)] [mod-name (string->symbol mod-name)])
    (when (hash-has-key? modules mod-name)
      (error (format "Encountered duplicate module name ~a" mod-name)))
...
(define (get-module-attribute mod attr)
  (or (symbol? attr) (error (format "Module attribute ~a must be a symbol" attr)))
...
(module+ test
  ...
  (let* ([json-doc (make-lakeroad-json-doc)]
    ...
    (let* ([ms (hash-ref json-doc 'modules)] [m (get-module ms "test-module")])
      ...
      (check-exn (regexp ".*must be a symbol.*")
                 (lambda () (get-module-attribute m "hdlname"))
                 "my-module")
...
(define (get-cell-port-direction cell port)
  (hash-ref (hash-ref cell 'port_directions) port))

(define (get-cell-port-connection cell port)
  (hash-ref (hash-ref cell 'connections) port))
