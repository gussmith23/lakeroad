...
(module+ test
      ...
      (system
       (format
        ...
        (build-path (getenv "LAKEROAD_DIR")
                    "verilog"
                    "pipelined_add_multiply_accumulate.sv")
        "pipelined_add_multiply_accumulate")))))
  (define ns (namespace-anchor->namespace anc))
  (define f (eval (first (btor->racket btor)) ns))

  (define-symbolic a2 b2 c2 acc2 (bitvector 16))
  ...
  (define dsp-a (zero-extend (choose a2 b2 c2 acc2 (bv 0 1)) (bitvector 30)))
  (define dsp-b (zero-extend (choose a2 b2 c2 acc2 (bv 0 1)) (bitvector 18)))
  (define dsp-c (zero-extend (choose a2 b2 c2 acc2 (bv 0 1)) (bitvector 48)))
  (define dsp-d (zero-extend (choose a2 b2 c2 acc2 (bv 0 1)) (bitvector 27)))
