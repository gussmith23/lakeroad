...
(define xilinx-ultrascale-plus-cfglut5
  (λ (#:CDI
    ...
    (let* ((merged-input-state-hash (list))
           (init-hash
            (append
             (list (cons 'state18 (bv 1 (bitvector 1))))
             (append
              (list (cons 'state15 (bv 0 (bitvector 32))))
              (append (list (cons 'state13 (bv 0 (bitvector 32)))) (list)))))
           (btor1 (bitvector 1))
           (btor2 CDI)
           ...
           (btor10 IS_CLK_INVERTED)
           (merged-input-state-hash
            (append merged-input-state-hash (signal-state IS_CLK_INVERTED)))
           (btor11 (bitvector 32))
           (btor12 (bv->signal (bv 0 (bitvector 32))))
           (btor13
            (let* ((state-value
                    (cond
                     ((assoc-has-key? merged-input-state-hash 'state13)
                      (bv->signal
                       (assoc-ref merged-input-state-hash 'state13)))
                     ((assoc-has-key? init-hash 'state13)
                      (bv->signal (assoc-ref init-hash 'state13)))
                     (else
                      (bv->signal
                       ((lambda ()
                          (log-warning
           ...
           (btor15
            (let* ((state-value
                    (cond
                     ((assoc-has-key? merged-input-state-hash 'state15)
                      (bv->signal
                       (assoc-ref merged-input-state-hash 'state15)))
                     ((assoc-has-key? init-hash 'state15)
                      (bv->signal (assoc-ref init-hash 'state15)))
                     (else
                      (bv->signal
                       ((lambda ()
                          (log-warning
           ...
           (btor18
            (let* ((state-value
                    (cond
                     ((assoc-has-key? merged-input-state-hash 'state18)
                      (bv->signal
                       (assoc-ref merged-input-state-hash 'state18)))
                     ((assoc-has-key? init-hash 'state18)
                      (bv->signal (assoc-ref init-hash 'state18)))
                     (else
                      (bv->signal
                       ((lambda ()
                          (log-warning
           ...
           (btor21
            (signal
             (concat (signal-value btor18) (signal-value btor4))
             (list)))
           (btor22 (bv->signal (bv 1 (bitvector 2))))
           (btor23
            (signal
             (bool->bitvector
           ...
           (btor45
            (bv->signal
             ...
             btor24))
           (btor46 (bv->signal (bv 0 (bitvector 1))))
           (btor47
            (bv->signal
           ...
           (btor51
            (signal
             (concat (signal-value btor50) (signal-value btor2))
             (list)))
           (btor52 (if (bitvector->bool (signal-value btor3)) btor51 btor24))
           (btor53 (if (bitvector->bool (signal-value btor10)) btor24 btor52)))
      (list
       (cons
        'O5
        (signal
         (signal-value btor34)
         (append
          (list (cons 'state18 (signal-value btor4)))
          (append
           (list (cons 'state15 (signal-value btor53)))
           (append (list (cons 'state13 (signal-value btor24))) (list))))))
       (cons
        'CDO
        (signal
         (signal-value btor25)
