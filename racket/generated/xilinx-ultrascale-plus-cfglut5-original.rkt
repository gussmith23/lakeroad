...
(define xilinx-ultrascale-plus-cfglut5
  (λ (#:CDI
    ...
    (let* ((merged-input-state-hash (list))
           (init-hash
            (append
             (list (cons 'state18 (bv 0 (bitvector 1))))
             (append
           ...
           (btor15
            (let* ((state-value
              ...
              (when (not (signal? state-value)) (error "Expected signal"))
              state-value))
           (btor17 (bv->signal (bv 0 (bitvector 1))))
           (btor18
            (let* ((state-value
                    (cond
           ...
           (btor50
            (signal
             ...
             (list)))
           (btor51 (if (bitvector->bool (signal-value btor3)) btor50 btor24))
           (btor52 (if (bitvector->bool (signal-value btor10)) btor51 btor24)))
      (list
       (cons
        'O5
        (signal
         ...
         (append
          ...
          (append
           (list (cons 'state15 (signal-value btor52)))
