...
(module+ test
  ...
  (test-case "Synthesize == and != implementation with a single ripple PFU"
    (begin
      (define lakeroad-expr
        (let* (;;; Lattice PFUs are groups of 8 4-input LUTs, side by side. To feed inputs into a PFU,
               ...
               [padded-inputs (lr:list (list (lr:bv (bv->signal (zero-extend a (bitvector 8))))
               ...
               [ripple-pfu-lutmem (lr:bv (bv->signal (?? (bitvector 16))))]
               ;;; A symbolic value representing the carry-in to the ripple PFU.
               [ripple-pfu-cin (lr:bv (bv->signal (?? (bitvector 1))))]
  ...
  (test-case "Synthesize <=, <, >=, > implementations with three ripple PFUs"
    (begin
      (define lakeroad-expr
