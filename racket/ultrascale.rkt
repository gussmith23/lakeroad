    ...
    (signal-value
     (hash-ref
      (interpret-xilinx-ultrascale-plus-dsp48e2
       (list (cons 'A (bv->signal (interpreter A)))
             (cons 'ACASCREG (bv->signal (interpreter ACASCREG)))
             (cons 'ACIN (bv->signal (interpreter ACIN)))
             (cons 'ADREG (bv->signal (interpreter ADREG)))
             (cons 'ALUMODE (bv->signal (interpreter ALUMODE)))
             (cons 'ALUMODEREG (bv->signal (interpreter ALUMODEREG)))
             (cons 'AMULTSEL (bv->signal (interpreter AMULTSEL)))
             (cons 'AREG (bv->signal (interpreter AREG)))
             (cons 'AUTORESET_PATDET (bv->signal (interpreter AUTORESET_PATDET)))
             (cons 'AUTORESET_PRIORITY (bv->signal (interpreter AUTORESET_PRIORITY)))
             (cons 'A_INPUT (bv->signal (interpreter A_INPUT)))
             (cons 'B (bv->signal (interpreter B)))
             (cons 'BCASCREG (bv->signal (interpreter BCASCREG)))
             (cons 'BCIN (bv->signal (interpreter BCIN)))
             (cons 'BMULTSEL (bv->signal (interpreter BMULTSEL)))
             (cons 'BREG (bv->signal (interpreter BREG)))
             (cons 'B_INPUT (bv->signal (interpreter B_INPUT)))
             (cons 'C (bv->signal (interpreter C)))
             (cons 'CARRYCASCIN (bv->signal (interpreter CARRYCASCIN)))
             (cons 'CARRYIN (bv->signal (interpreter CARRYIN)))
             (cons 'CARRYINREG (bv->signal (interpreter CARRYINREG)))
             (cons 'CARRYINSEL (bv->signal (interpreter CARRYINSEL)))
             (cons 'CARRYINSELREG (bv->signal (interpreter CARRYINSELREG)))
             (cons 'CEA1 (bv->signal (interpreter CEA1)))
             (cons 'CEA2 (bv->signal (interpreter CEA2)))
             (cons 'CEAD (bv->signal (interpreter CEAD)))
             (cons 'CEALUMODE (bv->signal (interpreter CEALUMODE)))
             (cons 'CEB1 (bv->signal (interpreter CEB1)))
             (cons 'CEB2 (bv->signal (interpreter CEB2)))
             (cons 'CEC (bv->signal (interpreter CEC)))
             (cons 'CECARRYIN (bv->signal (interpreter CECARRYIN)))
             (cons 'CECTRL (bv->signal (interpreter CECTRL)))
             (cons 'CED (bv->signal (interpreter CED)))
             (cons 'CEINMODE (bv->signal (interpreter CEINMODE)))
             (cons 'CEM (bv->signal (interpreter CEM)))
             (cons 'CEP (bv->signal (interpreter CEP)))
             (cons 'CLK (bv->signal (interpreter CLK)))
             (cons 'CREG (bv->signal (interpreter CREG)))
             (cons 'D (bv->signal (interpreter D)))
             (cons 'DREG (bv->signal (interpreter DREG)))
             (cons 'INMODE (bv->signal (interpreter INMODE)))
             (cons 'INMODEREG (bv->signal (interpreter INMODEREG)))
             (cons 'IS_ALUMODE_INVERTED (bv->signal (interpreter IS_ALUMODE_INVERTED)))
             (cons 'IS_CARRYIN_INVERTED (bv->signal (interpreter IS_CARRYIN_INVERTED)))
             (cons 'IS_CLK_INVERTED (bv->signal (interpreter IS_CLK_INVERTED)))
             (cons 'IS_INMODE_INVERTED (bv->signal (interpreter IS_INMODE_INVERTED)))
             (cons 'IS_OPMODE_INVERTED (bv->signal (interpreter IS_OPMODE_INVERTED)))
             (cons 'IS_RSTALLCARRYIN_INVERTED (bv->signal (interpreter IS_RSTALLCARRYIN_INVERTED)))
             (cons 'IS_RSTALUMODE_INVERTED (bv->signal (interpreter IS_RSTALUMODE_INVERTED)))
             (cons 'IS_RSTA_INVERTED (bv->signal (interpreter IS_RSTA_INVERTED)))
             (cons 'IS_RSTB_INVERTED (bv->signal (interpreter IS_RSTB_INVERTED)))
             (cons 'IS_RSTCTRL_INVERTED (bv->signal (interpreter IS_RSTCTRL_INVERTED)))
             (cons 'IS_RSTC_INVERTED (bv->signal (interpreter IS_RSTC_INVERTED)))
             (cons 'IS_RSTD_INVERTED (bv->signal (interpreter IS_RSTD_INVERTED)))
             (cons 'IS_RSTINMODE_INVERTED (bv->signal (interpreter IS_RSTINMODE_INVERTED)))
             (cons 'IS_RSTM_INVERTED (bv->signal (interpreter IS_RSTM_INVERTED)))
             (cons 'IS_RSTP_INVERTED (bv->signal (interpreter IS_RSTP_INVERTED)))
             (cons 'MASK (bv->signal (interpreter MASK)))
             (cons 'MREG (bv->signal (interpreter MREG)))
             (cons 'MULTSIGNIN (bv->signal (interpreter MULTSIGNIN)))
             (cons 'OPMODE (bv->signal (interpreter OPMODE)))
             (cons 'OPMODEREG (bv->signal (interpreter OPMODEREG)))
             (cons 'PATTERN (bv->signal (interpreter PATTERN)))
             (cons 'PCIN (bv->signal (interpreter PCIN)))
             (cons 'PREADDINSEL (bv->signal (interpreter PREADDINSEL)))
             (cons 'PREG (bv->signal (interpreter PREG)))
             (cons 'RND (bv->signal (interpreter RND)))
             (cons 'RSTA (bv->signal (interpreter RSTA)))
             (cons 'RSTALLCARRYIN (bv->signal (interpreter RSTALLCARRYIN)))
             (cons 'RSTALUMODE (bv->signal (interpreter RSTALUMODE)))
             (cons 'RSTB (bv->signal (interpreter RSTB)))
             (cons 'RSTC (bv->signal (interpreter RSTC)))
             (cons 'RSTCTRL (bv->signal (interpreter RSTCTRL)))
             (cons 'RSTD (bv->signal (interpreter RSTD)))
             (cons 'RSTINMODE (bv->signal (interpreter RSTINMODE)))
             (cons 'RSTM (bv->signal (interpreter RSTM)))
             (cons 'RSTP (bv->signal (interpreter RSTP)))
             (cons 'SEL_MASK (bv->signal (interpreter SEL_MASK)))
             (cons 'SEL_PATTERN (bv->signal (interpreter SEL_PATTERN)))
             (cons 'USE_MULT (bv->signal (interpreter USE_MULT)))
             (cons 'USE_PATTERN_DETECT (bv->signal (interpreter USE_PATTERN_DETECT)))
             (cons 'USE_SIMD (bv->signal (interpreter USE_SIMD)))
             (cons 'USE_WIDEXOR (bv->signal (interpreter USE_WIDEXOR)))
             (cons 'XORSIMD (bv->signal (interpreter XORSIMD)))
             (cons 'unnamed-input-331 (bv->signal (interpreter unnamed-input-331)))
             (cons 'unnamed-input-488 (bv->signal (interpreter unnamed-input-488)))
             (cons 'unnamed-input-750 (bv->signal (interpreter unnamed-input-750)))
             (cons 'unnamed-input-806 (bv->signal (interpreter unnamed-input-806)))
             (cons 'unnamed-input-850 (bv->signal (interpreter unnamed-input-850)))))
      'P)))
