...
(define (lut nbits architecture logical-inputs lutmems outwidth)
  (let* (;;; Max bitwidth, between all inputs and the output.
         ...
         [logical-inputs (map (lambda (v)
                                (choose* (lr:zero-extend (lr:bv v) (lr:bitvector (bitvector max-bw)))
                                         (lr:dup-extend (lr:bv v) (lr:bitvector (bitvector max-bw)))))
                              logical-inputs)]
          ...
          (let* ([physical-inputs (logical-to-physical-mapping
                 ...
                 [physical-outputs (for/list ([i max-bw])
                                     (lr:lut (length logical-inputs)
                                             ...
                                             architecture
                                             (lr:bv lutmem)
                                             (lr:list-ref physical-inputs (lr:integer i))))])
            (lr:extract (lr:integer (sub1 outwidth))
                        (lr:integer 0)
                        (lr:first (physical-to-logical-mapping
                                   (choose* (ptol-bitwise) (ptol-bitwise-reverse))
                                   (lr:list physical-outputs)))))])
...
(define (comparison nbits architecture logical-inputs lutmems)
  (let* (;;; First: generate two of the simple LUT template.
         ...
         [output (lr:list-ref lakeroad-expr-carry (lr:integer 1))])
    output))
