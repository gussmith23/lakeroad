...
(module+ test
  ...
  (require (only-in rosette synthesize bitvector zero-extend bvmul define-symbolic))
  (test-case
   "Generate code for DSP48E2 and do synthesis with it."
   (begin
     (match-define (list (hash-table ('P (list P-function P-contract))) requires)
       (btor->racket (file->string dsp-btor-file)))
     (define ns (namespace-anchor->namespace anc))
     ;;;(define/contract P (eval P-contract ns) (eval P-function ns))
     (define P (time (eval P-function ns)))

     (with-output-to-file "tmp.rkt" (thunk (pretty-write P-function)))

     (time (P))

     ;;; DSPs implement 28x18 signed multiplication. Rosette's bvmul is unsigned multiplication.
     ;;; We do unsigned multiplication on the DSP by limiting the input bitwidths to 26 and 17, which
     ;;; ensures the MSBs are 0.
     (define-symbolic a (bitvector 26))
     (define-symbolic b (bitvector 17))

     ;;; Unnamed inputs in the design. These are strange artifacts of Yosys. To ensure they don't
     ;;; matter, we include them in the #:forall clause of synthesis, which makes sure that our DSP
     ;;; implements the requested function no matter the value of these variables.
     (define-symbolic unnamed-input-897
                      unnamed-input-1331
                      unnamed-input-531
                      unnamed-input-1274
                      (bitvector 48))
     (define-symbolic unnamed-input-1375 (bitvector 1))
     ...
     (define soln
       (synthesize
        ...
        (list
         a
         b
         ;;; We also need to include these inputs in the forall list, as we have no control over them.
         unnamed-input-897
         unnamed-input-1331
         unnamed-input-531
         unnamed-input-1274
         unnamed-input-1375)
        #:guarantee
        (assert (bveq (bvmul (zero-extend a (bitvector 48)) (zero-extend b (bitvector 48)))
                      (signal-value (P #:A (bv->signal (zero-extend a (bitvector 30)))
                                       #:B (bv->signal (zero-extend b (bitvector 18)))
                                       ;;; Set some signals to 0 to speed synthesis.
                                       #:ACIN (bv->signal (bv 0 (bitvector 30)))
                                       #:BCIN (bv->signal (bv 0 (bitvector 18)))
                                       #:C (bv->signal (bv 0 (bitvector 48)))
                                       #:D (bv->signal (bv 0 (bitvector 27)))))))))
     (check-true (sat? soln))

     (pretty-display (model soln)))))
  ...
  (test-case
   ...
   (begin
     ...
     (match-define (list (list 'A
       ...
       (parse-btor-new (file->string dsp-btor-file)))
